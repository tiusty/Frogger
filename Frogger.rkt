...
; grade : 63 / 64
; comments : overall great job, but don't call main in your code. 
; Nothing should start when I click
; run, but rather wait until I type it in (style) <-1>
; also, if you put a number like .1 after the on-tick it will play 
; slowable so that it's not
; impossible to win :)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; Note The speed is set as such so it is harder to beat 
; but I beat it so it isn't impossible. Change on-tick
; to change speed so that the check-expects do not get messed up
; also, it makes the game less buggy by not increasing the step 
; size too much
...
; A Obstacle is one of:
;   - (make-plank Number Number Direction)
;   - (make-turtle Number Number Direction)
;   - (make-vehicle Number Number Direction)
...
(define vehicle50 (make-vehicle 30 ROW10 'left))
(define vehicle51 (make-vehicle (+ SPACING 30) ROW10 'left))
(define vehicle52 (make-vehicle (+ (* SPACING 2) 30) ROW10 'left))
(define vehicle53 (make-vehicle (+ (* SPACING 3) 30) ROW10 'left))
...
(define plank21 (make-plank (+ SPACING 100) ROW3 'right))
(define plank22 (make-plank (+ (* SPACING 2) 100) ROW3 'right))
(define plank23 (make-plank (+ (* SPACING 3) 100) ROW3 'right))
...
;; BELOW is the WORLD to CALL for STARTING GAME
...
(define (drawGameOver w img)
  (cond
    ...
    [(or (game-over? w) 
         (hitSomeVehicule (world-vehicles w) (world-player w)) 
...
(check-expect (draw-obstacles LOP1 BG)
              (place-image PLANK
                           -20 ROW1
                           BG))
(check-expect (draw-obstacles LOT1 BG)
                              (place-image TURTLE 40 ROW2 BG))
...
(check-expect (move-obstacles LOV1)
              (list (make-vehicle 415 320 'left)))
(check-expect (move-obstacles LOV2)
              (list
               (make-vehicle 135 360 'right)
               (make-vehicle 415 320 'left))) 
(check-expect (move-obstacles LOT1)
              (list (make-turtle 35 120 'left)))
...
(check-expect (key-handler WORLD0 "up")
              (make-world
               (make-player 375 530 'up)
               (list
                (make-vehicle 130 360 'right)
                (make-vehicle 420 320 'left))
               empty
               empty)) 
(check-expect (key-handler WORLD0 "down")
              (make-world
               (make-player 375 570 'down)
               (list
                (make-vehicle 130 360 'right)
                (make-vehicle 420 320 'left))
               empty
...
(define (key-handler w key)
  (make-world (move-player (world-player w) key) 
              (world-vehicles w) (world-planks w) (world-turtles w)))
...
(check-expect (move-world WORLD5)
              (make-world
               (make-player 375 550 'up)
               (list
                (make-vehicle 135 360 'right)
                (make-vehicle 415 320 'left))
               (list (make-plank -15 80 'right))
               (list (make-turtle 35 120 'left))))

(define (move-world w)
  (make-world (movePlayerObstacle w) 
...
; Purpose:
; move the player when the player is on a plank or turtle
; Player World -> Player
(define WORLDCOL1 (make-world (make-player 10 10 'right) empty empty 
                              (list (make-turtle 10 10 'left))))
(define WORLDCOL2 (make-world (make-player 10 10 'right) empty 
                              (list (make-plank 10 10 'right)) empty))
(check-expect (movePlayerObstacle WORLD1)
              (make-player 375 550 'up))
(check-expect (movePlayerObstacle WORLDCOL1)
              (make-player 5 10 'right))
(check-expect (movePlayerObstacle WORLDCOL2)
              (make-player 15 10 'right))
(define (movePlayerObstacle w) 
  (cond
    [(onPlanks? (world-player w) (world-planks w))
     (make-player (+ (player-x (world-player w)) SPEED) 
                  (player-y (world-player w)) (player-dir (world-player w)))]
    [(onTurtles? (world-player w) (world-turtles w))
     (make-player (- (player-x (world-player w)) SPEED) 
                  (player-y (world-player w)) (player-dir (world-player w)))]
    [else (world-player w)]))
...
(define LOP2 (list plank10 (make-plank 1000 1000 'right)))
(define LOT3 (list turtle10 (make-turtle -50 -50 'left)))
(check-expect (pruneObstacles LOV3) 
              (list
               (make-vehicle 270 440 'right)
               (make-vehicle 340 400 'right)
               (make-vehicle 130 360 'right)
               (make-vehicle 592.5 320 'left)
               (make-vehicle 385 320 'left)
               (make-vehicle 177.5 320 'left)
               (make-vehicle -30 320 'left)))
...
(check-expect (pruneObstacles LOT1)
              (list (make-turtle 40 120 'left)))
(check-expect (pruneObstacles LOP1)
              (list (make-plank -20 80 'right)))
(check-expect (pruneObstacles LOP2)
              (list
               (make-plank -20 80 'right)
               (make-plank -40 1000 'right)))
(check-expect (pruneObstacles LOT3)
              (list
               (make-turtle 40 120 'left)
               (make-turtle 790 -50 'left)))
...
; Obstacle -> Obstacle
(check-expect (addObstacle vehicle0) (make-vehicle 790 320 'left))
(check-expect (addObstacle vehicle1) (make-vehicle -40 360 'right))
(check-expect (addObstacle plank10) (make-plank -40 80 'right))
(check-expect (addObstacle turtle10) (make-turtle 790 120 'left)) 
...
(define (hitVehicle LOV a-player)
  (and 
   (>= (+ (player-x a-player) (/ TRILEN 4)) 
       (- (vehicle-x LOV) (/ VEHICLELENGTH 2)))
   (<= (- (player-x a-player) (/ TRILEN 4)) 
       (+ (vehicle-x LOV) (/ VEHICLELENGTH 2)))
   (>= (+ (player-y a-player) (/ TRILEN 4)) 
       (- (vehicle-y LOV) (/ VEHICLEWIDTH 2)))
   (<= (- (player-y a-player) (/ TRILEN 4))  
       (+ (vehicle-y LOV) (/ VEHICLEWIDTH 2)))))
...
(define playerInRiverTest1 (make-player 120 120 'right))
(define playerInRiverTest2 (make-player 160 160 'right))
(define playerInRiverTest3 (make-player 140 200 'right))
(define playerInRiverTest4 (make-player 140 240 'right))
(define playerInRiverTest5 (make-player 80 80 'right))
(check-expect (inRiver? player1 WORLD4)
              false)
(check-expect (inRiver? player1 WORLD5)
              false)
(check-expect (inRiver? playerInRiverTest1 WORLD4) true)
(check-expect (inRiver? playerInRiverTest2 WORLD4) true)
(check-expect (inRiver? playerInRiverTest3 WORLD4) true)
(check-expect (inRiver? playerInRiverTest4 WORLD4) true)
(check-expect (inRiver? playerInRiverTest5 WORLD4) true)
...
(check-expect (onTurtle? playerTurtleTest turtle4) false)
(check-expect (onTurtle? playerTurtleTest turtle5) false)

(define (onTurtle? a-Player a-Turtle)
  (>= 1 (+ (/ (* (- (+ (player-x a-Player) (/ TRILEN 5)) 
                    (turtle-x a-Turtle)) 
                 (- (+ (player-x a-Player) (/ TRILEN 5)) 
                    (turtle-x a-Turtle)))
              (* (/ VEHICLELENGTH 2) (/ VEHICLELENGTH 2))) 
           (/ (* (- (+ (player-y a-Player) (/ TRILEN 5)) 
                    (turtle-y a-Turtle)) 
                 (- (+ (player-y a-Player) (/ TRILEN 5)) 
                    (turtle-y a-Turtle)))
              (* (/ VEHICLEWIDTH 2) (/ VEHICLEWIDTH 2))))))
; Purpose:
...
(define (onPlank? a-player a-Plank)
  (and 
   (>= (+ (player-x a-player) (/ TRILEN 4)) 
       (- (plank-x a-Plank) (/ VEHICLELENGTH 2)))
   (<= (- (player-x a-player) (/ TRILEN 4)) 
       (+ (plank-x a-Plank) (/ VEHICLELENGTH 2)))
   (>= (+ (player-y a-player) (/ TRILEN 4)) 
       (- (plank-y a-Plank) (/ VEHICLEWIDTH 2)))
   (<= (- (player-y a-player) (/ TRILEN 4))  
       (+ (plank-y a-Plank) (/ VEHICLEWIDTH 2)))))
...
(define (offBoard? a-player)
  (or
   (> (player-x a-player) (- WW (- TRILEN 4)))
   (> (player-y a-player) (- HH (- TRILEN 4)))
   (< (player-x a-player) (- TRILEN 4))))   
